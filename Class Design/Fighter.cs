 ...
 * - Used Constructor dependency injection for artillery
...
namespace FighterProgram
{
    ...
    public class Fighter
    {
        ...
        // Pre-condition: In the constructor, `row`, `column`, `strength`, `attackRange`, and `artilleryProvider` parameters must be non-negative.
        // Post-condition: Bind the properties to the parameter values.
        public Fighter(int row, int column, int strength, int attackRange, int[] artilleryProvider)
        {
            ...
            TargetsVanquished = 0;
            MinimumStrength = artilleryProvider[artilleryProvider.Length - 1];
        }
...
// - The object takes damage if target is unsuccessful
// - Target, shift and move are virtual to support overriding in derived classes
// - Minimum Strength is dependent on last value in the artillery array