...
namespace FighterProgram
{
     ...
     * 2. The initial mode of InfantryGuard is up (isUpMode = true).
     * 3. If the InfantryGuard is not alive, it cannot block.
     * 
     * Implementation Invariants:
     * 1. The 'shields' array holds the status of all the shields, non-negative integers.
     * 2. The 'isUpMode' boolean flag determines the mode of the InfantryGuard.
     */

    public class InfantryGuard : Infantry, IGuard
    {
        private int[] shields;
        ...
        // Postcondition: InfantryGuard and its base Infantry are initialized.
        public InfantryGuard(int row, int column, int strength, int attackRange, int[] artilleryProvider, int[] shields)
        ...
        // Precondition: None
        // Postcondition: Changes the mode of the InfantryGuard.
        private void ChangeMode()
        {
            isUpMode = !isUpMode;
        }

        // Precondition: The InfantryGuard should be alive and the input (x) should be a valid index in shields array.
        // Postcondition: If the InfantryGuard is in up mode and the shield at index x is viable, decreases the shield value by 1 and returns true.
        //                If the InfantryGuard is not in up mode, sets the shield at index x to 0 and returns true.
        //                If the InfantryGuard is not alive, returns false.
        ...
        /*
         * Precondition: InfantryGuard should be alive and inputs (x, y) should be valid targets. Also, shieldIndex should be a valid index in shields array.
